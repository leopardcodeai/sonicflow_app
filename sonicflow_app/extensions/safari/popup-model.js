...
export function upsertSiteRule(rules, nextRule) {
  const host = normalizeRuleHost(nextRule?.host);
  if (!host) {
    return rules;
  }
  ...
  const normalized = {
    host,
    mode: nextRule.mode ?? "focus",
    activityId: nextRule.activityId ?? "deep-work",
    autoStart: Boolean(nextRule.autoStart)
  };
  const existingIndex = rules.findIndex((rule) => normalizeRuleHost(rule.host) === host);

  if (existingIndex === -1) {
    return [...rules, normalized].sort((a, b) => a.host.localeCompare(b.host));
  }

  return rules.map((rule, index) => index === existingIndex ? normalized : rule);
}

export function deleteSiteRule(rules, host) {
  const normalizedHost = normalizeRuleHost(host);
  return rules.filter((rule) => normalizeRuleHost(rule.host) !== normalizedHost);
}

export function resolveRuleForUrl(rules, url) {
  const host = normalizeRuleHost(url);
