...
struct ModeCard: View {
    let mode: FlowMode
    let isSelected: Bool
    var language: MacSonicFlowLanguage = .english
    let onTap: () -> Void
    ...
    var body: some View {
        Button(action: onTap) {
            HStack(spacing: BrandTokens.Spacing.sm) {
                ...
                Spacer(minLength: BrandTokens.Spacing.xs)

                Image(systemName: isSelected ? "checkmark.circle.fill" : "circle")
                    .font(.caption.weight(.bold))
                    .foregroundStyle(isSelected ? mode.accentColor : BrandTokens.Neutral.muted.opacity(0.72))
            }
            .frame(maxWidth: .infinity, minHeight: 48, alignment: .leading)
            .padding(.horizontal, BrandTokens.Spacing.md)
            .padding(.vertical, BrandTokens.Spacing.sm)
            .background(BrandTokens.Neutral.panel.opacity(isSelected ? 0.78 : 0.52), in: RoundedRectangle(cornerRadius: BrandTokens.Radius.lg, style: .continuous))
            .overlay(
