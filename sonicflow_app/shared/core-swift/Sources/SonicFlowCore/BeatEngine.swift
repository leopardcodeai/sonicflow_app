...
public struct BeatEngine: Sendable {
    ...
    public func generate(
        ...
        let carrierAngularStep = Self.twoPi * profile.carrierHz / sampleRate
        let beatAngularStep = Self.twoPi * profile.targetBeatHz / sampleRate
        let spatial = profile.sleepSpatialization
        let spatialAngularStep = spatial.enabled ? Self.twoPi * spatial.rockingHz / sampleRate : 0
        let spatialNormalizer = spatial.enabled ? 1 + spatial.panDepth : 1
        let amplitudeGain = Self.amplitude * Float(profile.outputGain)

        for frame in 0..<frameCount {
            let carrier = sin(carrierAngularStep * Double(frame))
            let beatPhase = beatAngularStep * Double(frame)
            let leftModulation = amplitudeModulation(profile: profile, beatPhase: beatPhase, phaseOffset: 0)
            let rightModulation = amplitudeModulation(profile: profile, beatPhase: beatPhase, phaseOffset: profile.stereoPhaseOffset)
            let envelope = envelope(at: frame, totalFrames: frameCount, fadeFrames: fadeFrames)
            var leftSpatialGain: Float = 1
            var rightSpatialGain: Float = 1

            if spatial.enabled {
                let pan = spatial.panDepth * sin(spatialAngularStep * Double(frame))
                leftSpatialGain = Float((1 - pan) / spatialNormalizer)
                rightSpatialGain = Float((1 + pan) / spatialNormalizer)
            }

            channelData[0][frame] = Float(carrier * leftModulation) * amplitudeGain * envelope * leftSpatialGain
            channelData[1][frame] = Float(carrier * rightModulation) * amplitudeGain * envelope * rightSpatialGain
        }
    ...
    private func amplitudeModulation(profile: ModulationProfile, beatPhase: Double, phaseOffset: Double) -> Double {
        let lfo = 0.5 + 0.5 * sin(beatPhase + phaseOffset)
