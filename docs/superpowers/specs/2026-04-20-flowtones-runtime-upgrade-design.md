# SF-31 FlowTones Runtime Upgrade Design

## Summary

Upgrade SonicFlow across iOS, macOS/Safari app shell, Android, Chrome, and Safari extension surfaces using the latest standalone `FlowTones` module from `Sound-Healing-Assistant-iOS`.

The design goal is:
- adopt the newer FlowTones runtime concepts where each platform can support them
- move each product surface toward the standalone FlowTones UI
...
The upstream `FlowTones` module adds a richer runtime model than the current SonicFlow implementations:
- `FlowTonePreset`
- `FlowToneSettings`
- `FlowToneBeatEngine`
- `FlowToneAmbientEngine`
- `FlowToneRenderer`
- `FlowToneCache`
- `FlowToneExample`
...
1. Use upstream FlowTones semantics instead of inventing parallel models.
2. Keep the product name `SonicFlow`, while allowing `FlowTones` to remain in internal module paths where renaming would create churn.
...
All platforms should converge toward the standalone FlowTones app structure:
...
- FlowTones-style screen hierarchy
...
- FlowTones-style popup layout and content hierarchy
...
- SonicFlow uses the latest FlowTones runtime concepts where support is real.
- All user-facing platforms move toward the standalone FlowTones UI.
